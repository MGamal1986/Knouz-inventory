...
2. Click **Add Product** — opens a dialog: description, category, supplier, purchase date, original cost, quantity purchased, selling price, an optional discount (percentage or fixed EGP amount), an invoice image upload, and a **"Pay from capital"** toggle. Profit % is calculated live as you type ((selling price ÷ original cost − 1) × 100) — you never enter it directly. The "Pay from capital" toggle (on by default) decides whether this purchase draws down your working capital: when on, `original cost × quantity` is deducted from the Capital balance (§6) and the helper text shows the exact EGP amount; when off, the purchase is treated as an outside expense and capital is left untouched. It's only shown when adding a product, not when editing one.
...
4. To fix a mistake later: the pencil icon on any row reopens the same dialog prefilled, for both the details and a new invoice image if needed. **Quantity purchased is locked once the product exists** — it's shown greyed out in the edit dialog, since quantity is only ever changed one way from here on (see "Restocking a product" below). Everything else (description, category, supplier, cost, price, discount, invoice image) stays editable. The trash icon deletes the product — it disappears from Inventory, the dashboard's Sold Out Items section, and all stock/product counts immediately, and its code can never be reissued to a new product. Under the hood this is a soft delete: the row is kept (not removed) so past sales, invoices, and profit/revenue history involving that product are completely unaffected.
...
### 4.1 Restocking a product

Every row in **Inventory** has a **Restock** icon (next to Edit/Delete) — available for any product, whether it's still in stock, partially sold, or fully sold out. Click it to open a dialog with: quantity to add, and, since a new batch is often bought at a different price, editable **Original Cost**, **Profit %** (selling price recalculates live, same as the Add Product dialog), **Discount type/value** (all prefilled with the product's current values), and the same **"Pay from capital"** toggle as Add Product — on by default, it deducts this batch's cost (`original cost × quantity added`) from the Capital balance, or leaves capital untouched if you turn it off for an outside-funded restock. Confirming adds the quantity and — if you changed them — updates the cost, selling price, and discount going forward; the product's status (In Stock / Partially Sold / Sold Out) is recomputed automatically. The Dashboard's Sold Out Items section (see §6) is read-only now — it's a heads-up list, not where restocking happens.
...
7. Behind the scenes, in one transaction: stock is deducted, each product's status flips to In Stock / Partially Sold / Sold Out as appropriate, an invoice number is issued in the form `{ClientFirstName}-{Mobile}-{sequence}` (e.g. `Ahmed-01012345678-000008` — the trailing 6-digit sequence keeps it unique even across repeat sales to the same client), the sale total is added to the Capital balance (§6), and a PDF is generated using that same string as its filename.
...
2. Confirm — the system, in one transaction: adds the refunded quantity back to that product's available stock (at its existing cost/price — refunding never touches product pricing or discount config), records the refund, deducts the amount actually paid for those units (i.e. after any discount that applied) from revenue, and rolls that same amount back out of the Capital balance (§6) — the reverse of what the original sale had added.
...
- **Dashboard** (`/`) — 7 KPI tiles pulled live from the database: total products, total stock units, stock value, actual profit, sold-out count, sales this month, and **Capital**. Actual profit sums `(selling price − original cost)` across every unit ever sold, net of refunds — it's a running total of realized profit, unaffected by later deleting a product. **Capital** is the business's live working-capital balance: it starts from an opening balance, goes **up** by each sale's total, and **down** by each refund and by any purchase/restock funded "from capital" (see §4 / §4.1). Under the hood it's an append-only ledger — every one of those movements is recorded as its own signed entry and the balance is the sum of them all, so it's always auditable rather than a single number that gets overwritten. (Purchases/restocks toggled *off* "Pay from capital" don't affect it.) Below that, a **Revenue Explorer** card lets you filter revenue by time range (today/week/month/year/custom dates), category, client, or product, showing gross revenue, refunded amount, and net revenue for whatever's selected. Below that, a **Sold Out Items** section lists every product with zero stock left (paginated at 6/page) — read-only, for visibility; restock it from Inventory instead (see §4.1). Below that, a **Stock by Category** card grid shows units in stock and total original cost per category. Below that, a **Top Selling** card lists the top 5 products and top 5 categories by net units sold, all-time (refunded units excluded), each ranked #1–#5. "Add Item" jumps to Inventory.
