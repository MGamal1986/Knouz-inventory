...
Every row in **Inventory** has a **Restock** icon (next to Edit/Delete) — available for any product, whether it's still in stock, partially sold, or fully sold out. Click it to open a dialog with: quantity to add, and, since a new batch is often bought at a different price, editable **Original Cost**, **Profit %** (selling price recalculates live, same as the Add Product dialog), and **Discount type/value**, all prefilled with the product's current values. Confirming adds the quantity and — if you changed them — updates the cost, selling price, and discount going forward; the product's status (In Stock / Partially Sold / Sold Out) is recomputed automatically. The Dashboard's Sold Out Items section (see §6) is read-only now — it's a heads-up list, not where restocking happens.
